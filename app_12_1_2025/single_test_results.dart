...
/*
  Future<void> fetchTest() async {
    try {
      ...
      if (snapshot.exists) {
        setState(() {
          int lastTest =snapshot.get('tests_numbers');
          String result =snapshot.get('test'+lastTest.toString());
          List<String> parts = result.split('+');
          FirstAttempt = int.parse(parts[0].trim());
          SecondAttempt = int.parse(parts[1].trim());
          ThirdAttempt = int.parse(parts[2].trim());
          print(FirstAttempt);
          print(SecondAttempt);
          print(ThirdAttempt);
          best_score= max(max(FirstAttempt,SecondAttempt),ThirdAttempt);
           if (FirstAttempt==5) {
          ...
          success_rate=(FirstAttempt+SecondAttempt+ThirdAttempt)/15;
          average_score= success_rate * 5;
        }
        );
      }
    } catch (e) {
