...
// Function to show no Wi-Fi connection dialog
    ...
    try {
      ...
      for (var doc in querySnapshot.docs) {
       ...
       // goal_bestScore.add(int.parse(goalsParts[3].trim()));
        goal_bestScore.add(max(max(int.parse(goalsParts[0].trim()), int.parse(goalsParts[1].trim())),
            int.parse(goalsParts[2].trim())));
     ...
     /*   if (curr_firstAttempt[i] == 5) {
        ...
        for(int k =0; k<testNumbers;k++) {
          String allresult = data['test' + (k + 1).toString()];
          List<String> allparts = allresult.split('+');
          if (int.parse(allparts[0].trim()) == 5) {
            times_perfect_score += 1;
          }
          if (int.parse(allparts[1].trim()) == 5) {
            times_perfect_score += 1;
          }
          if (int.parse(allparts[2].trim()) == 5) {
            times_perfect_score += 1;
          }
        }
        curr_timesPerfectScore.add(times_perfect_score);
  ...
  Widget _buildSummary(String title, String current, String goal,String extension) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Column(
      ...
      child: Container(
        ...
        child: Column(
          ...
          IconButton(
            ...
            _buildSummary('First Attempt', curr_firstAttempt[i].toString(), goal_firstAttempt[i].toString(),'/5'),
            _buildSummary('Second Attempt', curr_secondAttempt[i].toString(), goal_secondAttempt[i].toString(),'/5'),
            _buildSummary('Third Attempt', curr_thirdAttempt[i].toString(), goal_thirdAttempt[i].toString(),'/5'),
            _buildSummary('Best Score', curr_bestScore[i].toString(), goal_bestScore[i].toString(),'/5'),
            _buildSummary('Times Perfect Score', curr_timesPerfectScore[i].toString(), goal_timesPerfectScore[i].toString(),' '),
            _buildSummary('Average Score', curr_averageScore[i].toString(), goal_averageScore[i].toString(),'/5.0'),
